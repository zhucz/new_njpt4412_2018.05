...
static void i2c_init (int speed, int slaveadd)
{
	int i, status;
		  ...
		  unsigned char data[], unsigned short data_len)
{
	...
	case I2C_READ:
		...
		if (IsACK ()) {			
				...
				rIICDS = (chip + 1);
				debug("187: chip = %#x \n",chip + 1);
				rIICSTAT =  I2C_MODE_MR | I2C_TXRX_ENA | I2C_START_STOP;
...
void lowlevel_init_pmic8767(unsigned char Address,unsigned char *Val,int flag)
{
	unsigned char addr;
	...
	if(flag == 0)
	{
		...
		*Val = data[0];
		debug("max8997: addr 0x%x, value 0x%x ret %d\n",addr,data[0],ret);
	}
	else 
	{
		...
		ret = i2c_read(MAX8997_I2C_ADDR, addr, 1, data, 1);
		*Val = data[0];
		debug("max8997: addr 0x%x,write value 0x%x,read value 0x%x ret %d\n",addr,write_data,data[0],ret);
	}
...
//void pmic_s5m8768a_read_id(void)
//{
//	i2c_write(MAX8997_I2C_ADDR, addr, 1, data, 1);
