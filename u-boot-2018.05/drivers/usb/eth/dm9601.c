...
//   printf("dm9601_mdio_read() phy_id=0x%02x, loc=0x%02x, returns=0x%04x|0x%04x\n",phy_id, loc, *v, le16_to_cpu(*v));
...
//   printf("dm9601_mdio_write() phy_id=0x%02x, loc=0x%02x, val=0x%04x|0x%04x\n",phy_id, loc, res, val);
...
//   printf("\n****************** %s *******************\n", __func__);
   ...
   for (i = 0; products[i].vendor != 0; i++) {
       ...
       if (dev->descriptor.idVendor == products[i].vendor && dev->descriptor.idProduct == products[i].product)
...
int dm9601_eth_get_info(struct usb_device *usb_dev, struct ueth_data *ss, struct eth_device *eth)
{
   u8 id = 0xff;
   u8 mcast0[DM_MCAST_SIZE] = { 0x0 };
...
//   printf("%s%d", DM9601_BASE_NAME, 0 /*curr_eth_dev++*/);
	
//	writel(0x2, 0x11400064);  //GPC0DAT
...
//       dm_write_reg(ss, DM_MODE_CTRL, mode | 0x80); /*add by zhuchengzhi 2018-07-14  */
