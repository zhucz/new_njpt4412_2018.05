...
#else  /* End CONFIG_DM_ETH */

typedef void (*usb_eth_before_probe)(void);
typedef int (*usb_eth_probe)(struct usb_device *dev, unsigned int ifnum, struct ueth_data *ss);
typedef int (*usb_eth_get_info)(struct usb_device *dev, struct ueth_data *ss, struct eth_device *dev_desc);
...
#endif
/*-----------add by zhuchengzhi 2018-06-03 TODO--------------------------*/
...
//			printf("i=%d, %s\n", i, dev ? dev->dev->name : "(done)");
			if (!dev){
				printf("no more devices available ..... zhuchengzhi\n");
...
//			printf("probe_valid_drivers ....  zhuchengzhi \n");
			/* check limit */
